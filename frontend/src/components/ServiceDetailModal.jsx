...
import {
  Play, Square, RotateCw, Container, LayoutGrid, ScrollText, FolderOpen,
  TerminalSquare, HardDrive, Archive, Settings,
} from 'lucide-react';
...
export default function ServiceDetailModal({ open, onClose, serviceId, onChanged }) {
  ...
  // Setup state (mantido aqui pra poder ser visto em outras abas também)
  const [setupState, setSetupState] = useState({ status: 'idle', step: 'idle', progress: 0, error: '', running: false });
  const [setupLogs, setSetupLogs] = useState([]);
  const setupLogsRef = useRef(setupLogs);
  setupLogsRef.current = setupLogs;
  ...
  useEffect(() => {
    ...
    setTab('overview');
    setDiskUsage(null);
    setSetupState({ status: 'idle', step: 'idle', progress: 0, error: '', running: false });
    setSetupLogs([]);

    let cancelled = false;

    // Carrega dados do serviço + estado de setup em paralelo.
    Promise.all([
      api.getService(serviceId),
      api.serviceSetup(serviceId).catch(() => null),
    ]).then(([data, setup]) => {
      if (cancelled) return;
      setService(data);
      ...
      if (setup) {
        setSetupState({
          ...
          step: setup.step || 'idle',
          progress: setup.progress || 0,
          error: setup.error || '',
          running: setup.running || false,
          startedAt: setup.startedAt,
          finishedAt: setup.finishedAt,
        });
        setSetupLogs(setup.logs || []);
      }
    }).catch((e) => { if (!cancelled) notify(e.message, 'error'); });

    // Socket: eventos em tempo real de setup.
    ...
    const onStatus = (payload) => {
      if (payload.serviceId !== serviceId) return;
      setService((s) => s ? { ...s, status: payload.status } : s);
    };
    socket?.on('service:setup', onSetup);
    socket?.on('service:setup-log', onSetupLog);
    socket?.on('service:status', onStatus);

    return () => {
      cancelled = true;
      socket?.off('service:setup', onSetup);
      socket?.off('service:setup-log', onSetupLog);
  ...
  const refresh = async () => {
    try {
      const [fresh, setup] = await Promise.all([
        api.getService(serviceId),
        api.serviceSetup(serviceId).catch(() => null),
      ]);
      setService(fresh);
  ...
  const runSetup = async ({ autoStart = true } = {}) => {
    setBusy(true);
    try {
      await api.runServiceSetup(serviceId, { auto_start: autoStart });
      notify('Setup iniciado', 'success');
    } catch (e) {
      notify(e.message, 'error');
    } finally {
      setBusy(false);
  ...
  const setupStepLabel = SETUP_STEP_LABELS[setupState.step] || setupState.step || 'Aguardando';
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4">
            ...
            {setupState.running && (
              <span className="text-xs text-signal animate-pulse">• setup em andamento — {setupStepLabel}</span>
            )}
            {setupState.status === 'failed' && (
              <span className="text-xs text-error">• setup falhou</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 border-b border-line-soft -mx-1 px-1 overflow-x-auto">
          {TABS.map((t) => {
            const Icon = t.icon;
            ...
            return (
              ...
              >
                ...
                {t.id === 'config' && setupState.status === 'failed' && (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            ...
            {!service.public_url && service.tunnel_hostname && (
              <div className="bg-signal-soft rounded-lg p-3 col-span-2">
                ...
                <p className="text-xs text-ink-faint mt-1">Ativo assim que o túnel nomeado estiver rodando.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            ...
            {isDocker && (
              <p className="text-xs text-ink-faint mt-2">
                Para enviar comandos a um container, use a aba <strong>Terminal</strong>.
              </p>
            )}
          </div>
        )}
