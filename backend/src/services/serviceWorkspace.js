...
function inferDockerCommand(image, command) {
  ...
  if (imageName.includes('node') || imageName.includes('npm')) {
    return "sh -c 'cd /app && if [ -f index.js ]; then node index.js; elif [ -f server.js ]; then node server.js; else echo \"Nenhum entrypoint encontrado (index.js ou server.js) em /app\"; exit 1; fi'";
  }
  return '';
}
