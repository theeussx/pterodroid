...
<a href="#sobre-o-projeto">Sobre</a> •
    ...
    <a href="#acesso-remoto-e-domínios-personalizados">Acesso Remoto</a> •
...
## 🌍 Acesso Remoto e Domínios Personalizados

O Pterodroid integra-se com o **Cloudflare Tunnel** para permitir que você exponha seus serviços e o próprio painel à internet de forma segura, sem a necessidade de abrir portas no seu roteador. Existem duas abordagens principais para configurar o acesso remoto:

### 1. Túnel Rápido (Quick Tunnel)

Ideal para acesso temporário ou para testar serviços rapidamente. O Pterodroid pode iniciar um túnel rápido que gera uma URL pública aleatória (`*.trycloudflare.com`).

> [!WARNING]A URL gerada por um Túnel Rápido **não é persistente** e muda a cada reinício do túnel. É recomendado apenas para testes ou uso esporádico. **Importante:** Túneis rápidos são adequados apenas para serviços HTTP/HTTPS. Bancos de dados não podem ser acessados via Quick Tunnels.

### 2. Domínios Personalizados (Named Tunnel)

Para um acesso mais profissional e persistente, você pode configurar um túnel nomeado com um domínio próprio (ex: `painel.seusite.com`). O Pterodroid oferece duas maneiras de gerenciar isso:

#### Opção A: Túnel Gerenciado pelo Painel (CLI-managed)

Neste modo, o Pterodroid automatiza a criação do túnel, a geração do arquivo de configuração (`config.yml`) e o roteamento DNS (registros `CNAME` no Cloudflare). Para usar esta opção, você precisará:
...
Uma alternativa mais simples para conectar, onde o túnel é criado e configurado diretamente no dashboard **Cloudflare Zero Trust** (Networks → Tunnels). O Cloudflare fornecerá um token que você pode colar no Pterodroid.

> [!IMPORTANT]Neste modo, o roteamento de cada domínio (qual hostname vai para qual porta) **deve ser configurado diretamente no dashboard da Cloudflare**, na aba "Public Hostname" do túnel. Os campos de domínio nos formulários de serviço/banco do Pterodroid não se aplicam aqui. Para acessar bancos de dados via Named Tunnel, o cliente também precisará usar `cloudflared access tcp`.
...
| **Gerenciamento de Arquivos** | `fs`, `path` do Node.js, `multer` | Manipulação segura de arquivos com validação de caminho, upload/download, edição e auditoria. `multer` para uploads multipart. |
| **Acesso Remoto** | `cloudflared` | Ferramenta CLI para criar túneis seguros e expor serviços à internet sem abrir portas. |
| **Autenticação** | JWT, `bcryptjs` | Segurança robusta para autenticação de usuários e hashing de senhas, sem dependências nativas. |
| **Monitoramento** | `/proc`, `/sys/class/thermal`, `ps`, `df` | Leitura direta de informações do sistema para CPU, RAM, disco, rede, temperatura e processos ativos. |
